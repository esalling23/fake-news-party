...
class FakeNews {
	...
	constructor() {
	  ...
	  Initialize(session) {
		...
		this.Profiles.find({})
		.populate('pros cons missions')
		.exec((err, result) => {
			console.log(result, " are all the profiles");
			...
			this.currentMission = result.missions[_.random(0, result.missions.length)];
			console.log(this.currentMission, " is the random mission");

			this.Scenario.find({ _id: this.currentMission.id })
			.populate('events')
			.exec((err, result) => {

				result.events.populate('articles');
				this.Articles.find({})
			});
	 ...
	 StartGame(socket, initial) {
		...
		let data = {
		    ...
		    pros: this.currentBuckets,
		    cons: this.currentBuckets, 
